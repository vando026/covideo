...
% \usepackage[natbib=true, style=nature, isbn=false, doi=false, url=false]{biblatex}
...
<<echo=FALSE>>=
home <- file.path(Sys.getenv("HOME"), "Seafile\\Heidelberg\\Projects\\CoVideo\\Paper_2")
...
\title{List experiment and behavioral intent COVID-19: an online randomized controlled trial of \Sexpr{fmt(flow$In)} adults in the
...
<<tabs, child='tables.Rnw', eval=FALSE>>=
@

<<tabs, child='figures.Rnw', eval=FALSE>>=
@
