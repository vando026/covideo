...
statements about their behavioral intent toward preventive COVID-19 practices. We also asked
direct verions of these statements, thereby enabling us to quantify the magnitude of
...
the do-nothing arm. We have The APC is also a wordless, animated video with the same duration as the
...
After the list experiments, participants were asked the five sensitive items directly. For
example, ``Please answer True or False if you agree with the following statement: This
week I will go out with my friends.'' To ensure post-trial access to
treatment,\citep{Doval2015} participants in the APC and do-nothing arm could choose to
watch the E-E video at the end of the study. 
...
\((i=1,\dots, 5)\). Let the superscripts \(^{ee}\) denote the E-E CoVideo, \(^{apc}\)
...
total, content, and attention effects of the CoVideo.  Let \(D_i^{Tot}\) denote the total effect,
which is estimated by \(P_{i}^{ee}-P_{i}^{no}\); let \(D_i^{Cont}\) denote the content
effect, which is estimated by \(P_{i}^{ee}-P_{i}^{apc}\); and let \(D_i^{Att}\) denote
the attention effect, which is estimated by \(P_{i}^{apc}-P_{i}^{no}\).  These analyses
are analogous to difference-in-difference analyses, which we implemented by specifying
the main and interaction terms in an ordinary least squares (OLS) regression model.
The OLS equation for the \(i\)th list experiment is given as: 
...
To evaluate how behavioral intent varies by age, we used a nonlinear least-squares
estimator for the list experiment response and covariate data. Designed by Blaire
and Imai,\citep{Blair2012} the model assumes that the response has a Poisson-Binomial
distribution. In addition, we calculated the magnitude of social desirability bias. We did
this first by calculating the prevalence of True responses to the five sensitive
questions, which we asked directlt. We then obtained the difference of the indirect and
direct responses, which is the bias. 
...
arm and treatment group.  These mean scores can be used to calculate the prevalence of
intent (with 95\% confidence intervals [CI]) for the five preventive COVID-19
behaviors. In addition, the mean scores can be used to estimate the total effect (the
difference between the E-E video and do-nothing) and content effect (the difference
between the E-E video and the APC) along with their 95\% CIs and p-values. These results are
shown in Figure \ref{fig:behav}, which includes an example calculation.  Results show that
participants in the CoVideo arm had lower behavioral intent to go out with friends when
compared with the APC (content effect $=$ \Sexpr{fmt(ddat[["SocialDist"]][["est",
"trteq"]], 3)}, \Sexpr{pfmt(ddat[["SocialDist"]][["pval", "trteq"]])}) and do-nothing
(total effect $=$\Sexpr{fmt(ddat[["SocialDist"]][["est", "toteq"]], 3)},
\Sexpr{pfmt(ddat[["SocialDist"]][["pval", "toteq"]])}) arms. Participants in the CoVideo
...
participants in the CoVideo arm had a higher behavioral intent than the APC (content
...
\Sexpr{pfmt(ddat[["CleanDishes"]][["pval", "toteq"]])}) arms. The E-E video had no impact
on behavioral intent to prevent COVID-19 spread by cleaning kitchen counters after use. 




Figure 6 shows the di?erences in behavioral intent by age and gender for each of the ?ve statements.
...
To measure baseline intent toward preventive COVID-19 behaviors, we obtained the
difference between the treatment and control list for the do-nothing arm.  Figure
\ref{fig:behav} shows these results.  Participants had high intent to wash their hands
frequently (\Sexpr{wash$ctrdif.est}\%, 95\% CI:
\Sexpr{wash$ctrdif.lb}--\Sexpr{wash$ctrdif.ub}\%), with lower intent to clean dishes after
use (\Sexpr{dish$ctrdif.est}\%, 95\% CI: \Sexpr{dish$ctrdif.lb}-\Sexpr{dish$ctrdif.ub}\%)
and clean kitchen counters after use (\Sexpr{kitch$ctrdif.est}\%, 95\% CI:
\Sexpr{kitch$ctrdif.lb}--\Sexpr{kitch$ctrdif.ub}\%). Despite advice not to,
\Sexpr{stock$ctrdif.est}\% of participants intended to stockpile household supplies for a
month (95\% CI: \Sexpr{stock$ctrdif.lb}--\Sexpr{stock$ctrdif.ub}\%) and
\Sexpr{soc$ctrdif.est}\% said they would go out with friends under lockdown restrictions
(95\% CI: \Sexpr{soc$ctrdif.lb}--\Sexpr{soc$ctrdif.ub}\%).

Shows the prevalence of behavioral intent to undertake preventive COVID-19
practices in the do-nothing arm.  The list
...
supplies, and more likely to clean hands, surfaces, and kitchen counters to prevent COVID-10 spread.
