...
doReg <- function(RHS, dat) {
  function(LHS) {
    mod <- lm(as.formula(paste(LHS, RHS)), data=dat)
    cf <- round(coef(mod)[2]*100, 1)
    ci <- round(confint(mod)[2, ]*100, 1) 
    return(c(cf, ci))
  }
}
doReg <- doReg_(RHS = " ~   TreatList", bdat)
...
####################### PLot Behav means #############################
######################################################################
doReg2 <- doReg_(RHS = "~ -1 + VideoArm:TreatList", ldat)

doReg2("SocialDist")

plotBar <- function(dat) {
  barplot(as.matrix(dat[, c(1, 7, 3, 9, 5, 11)]))
}
plotBar(tablist[1, ])


tapply(ldat2$SocialDist, list(ldat2$VideoArm, ldat2$TreatList), mean)
tapply(ldat$SocialDist, list(ldat$VideoArm, ldat$TreatList), mean)

group_by(ldat, VideoArm, TreatList) %>% 
  summarize(Mean = mean(SocialDist))
