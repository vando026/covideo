...
\caption{Shows the age, gender, country of residence, and education status, and first language of
...
\caption{Frequency of item responses for knowledge of  COVID-19 by CoVideo, Attention
Placebo Control (APC), and Control arms. The percentage of correct answers is also give
($N = $ \Sexpr{fmt(flow$FinalN)}).}
