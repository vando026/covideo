...
correcta <- res_all$means["VideoArmControl"]/kn*100
@
...
\textbf{Background:} Entertainment-education (E-E) media can improve knowledge and
...
that prevent COVID-19 spread (knowledge spread). Using a list randomization approach to
remove social desirability bias, we also measured behavioral intent to prevent the spread
of COVID-19. We recruited online participants aged 18--59 years from the United States,
the United Kingdom, Germany, Spain, and Mexico. 
...
questions, and \Sexpr{fmt(correcta, 1)}\% of the combined questions. Overall knowledge was
...
in increasing basic clinical knowledge of COVID-19 and knowledge of behaviors to prevent COVID-19
spread. Our results support the use of E-E media on social media platforms to distribute
COVID-19 information to many people on a rapid scale without requiring physical contact. 
...
experiment, and 3) survey questions about COVID-19 knowledge.  We used an
online platform called Prolific Academic (ProA: \url{https://www.prolific.co}) to recruit
participants from the United States, the United Kingdom, Germany, Spain, and Mexico. ProA
recruited on a ``first come, first served'' basis and participants had to be 18 to 59
years of age (male, female, or other) and have current residence in the United States, the
United Kingdom, Germany, Spain, Mexico, or France.  Participants were excluded from the
study if they could not speak English, German, or Spanish. The trial was hosted and
deployed on Gorilla™ (\url{www.gorilla.sc}), which is a cloud platform that provides
versatile tools to undertake online, experimental, and behavioral
research.\autocite{Anwyl-Irvine2020}  
...
\Sexpr{fmt(res_all$means["VideoArmControl"], 1)} out of \Sexpr{akn} items
(\Sexpr{fmt(res_all$means["VideoArmControl"]/akn*100, 1)}\%) were correctly answered.
With this high baseline score, the CoVideo could
therefore only increase knowledge by a maximum of \Sexpr{fmt(akmax, 1)} points.  
Relative to the Control arm, the CoVideo increased the average response by
\Sexpr{fmt(akdiff, 1)} points (\Sexpr{(res_all$pvals[2, 1])}), which represents a
\Sexpr{fmt(akdiff, 1)}/\Sexpr{fmt(akmax, 1)} $=$ \Sexpr{fmt(akdiff/akmax*100, 1)}
...
\Sexpr{fmt(res_all$means["VideoArmPlacebo"]/akn*100, 1)}\%).  When we removed the
attention affect of the video medium, the CoVideo increased knowledge by
\Sexpr{fmt(akdiff2, 1)} points (\Sexpr{res_all$pvals[2,2]}), which represents a
\Sexpr{fmt(akdiff2, 1)}/\Sexpr{fmt(akmax2, 1)} $=$ \Sexpr{fmt(akdiff2/akmax2*100, 1)}
percentage point increase in overall knowledge. 
...
when compared with participants assigned to the Control arm (Figure \ref{fig:clinic}). In
the Control arm, the average items correctly answered was
...
percentage point increase in clinical knowledge.  We report that there was no difference
...
future E-E strategies to support the long-term COVID-19 response as countries ease
